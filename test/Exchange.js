...
describe('Exchange', () => {
  ...
  describe('Making orders', () => {
    ...
    describe('Failing orders', () => {
      ...
      it('Rejects if amount that user receives does not allow for a fee of intiger type (in smallest denomination)', async () => {
        // First, ensure that user1 has another T1 token to trade, so that function doesn't revert with the
        // 'Insufficient balance' error.
        await token1.connect(user1).approve(exchange.address, amount);
        await exchange.connect(user1).depositToken(token1.address, amount);
        //_amountGet = 42 leads to _feeAmount = _amountGet * feePercent / 100 = 42 * 10 / 100 = 4.2
        //Note that 4.2 is no longer of type uint and, therefore,an incorrect amount for _feeAmount.
        //All the other examples, we used so far, work because _amountGet = amount = tokens(1) = 10^18 is multiple of 100 = 10^2 and, therefore,
  ...
  describe('Order actions', () => {
    ...
    beforeEach(async () => {
      ...
      // user 2 deposits 2 T2 tokens to exchange
    ...
    describe('Filing Orders', () => {
      ...
      describe('Failing Trades', () => {
        ...
        it("Rejects if user doesn't have enough fund to fill the order", async () => {
          // user2 withdraws 1 T2
          await exchange
            .connect(user2)
            .withdrawToken(token2.address, tokens(1));
          // user2 tries to fill the order. i.e. user2 1.1 T2 to fill the order but only has 1 T2
        ...
        it('Rejects cancelled orders', async () => {
          ...
          // User2 fills the cancelled order
          await expect(exchange.connect(user2).fillOrder(1)).to.be.revertedWith(
